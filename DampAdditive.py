...
class DampAdditive():
            ...
            for betaAst in betaAst_list:
                for phi in phi_list:
                    ...
                    for i in range(1, len(data)):
                        level_cur = alpha * data[i] + (1 - alpha) * (level_list[i - 1] + phi * trend_list[i - 1])
                        level_list.append(level_cur)
                        trend_cur = betaAst * (level_list[i] - level_list[i - 1]) + (1 - betaAst) * phi * trend_list[i - 1]
                        trend_list.append(trend_cur)
                        pred_data_cur = level_list[i] + phi * trend_list[i]
    ...
    def predict(self, data, predict_len):
        ...
        trend_list = [0]
        for i in range(1, len(data)):
            level_cur = alpha_best * data[i] + (1 - alpha_best) * (level_list[i - 1] + phi_best * trend_list[i - 1])
            level_list.append(level_cur)
            trend_cur = betaAst_best * (level_list[i] - level_list[i - 1]) + (1 - betaAst_best) * phi_best * trend_list[i - 1]
            trend_list.append(trend_cur)
            pred_data_cur = level_list[i] + phi_best * trend_list[i]
            pred_data.append(pred_data_cur)
        for i in range(predict_len):
            prediction[i] = level_list[-1] + np.sum([phi_best**(j+1) for j in range(i)]) * trend_list[-1]
        return prediction
